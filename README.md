...
**Преимущества JWT:**                                                                                                                 
  ...
  - Целостность данных — подпись гарантирует, что токен не был изменён после выдачи сервером

**Минусы:**
  - Невозможность мгновенной инвалидации — пока токен не истёк, его нельзя просто "отозвать" на сервере (нужен blacklist или
  короткий TTL)
  - Размер — JWT значительно больше обычного session ID (особенно с кастомными claims), что увеличивает размер каждого запроса
  - Нужно контролировать срок жизни — короткие токены безопаснее, но неудобнее; часто используют refresh tokens
  - Уязвимость к CSRF при хранении в httpOnly cookie (рекомендуемый способ) — браузер автоматически отправляет cookie, поэтому
  требуется дополнительная защита (например, XSRF-токен)
  - Уязвимость к XSS при хранении в localStorage — вредоносный скрипт может прочитать и украсть токен
  - Неправильная реализация может быть уязвима (например, [`алгоритм none`](https://portswigger.net/kb/issues/00200901_jwt-none-algorithm-supported) , слабый секрет подписи)
...
Реализации всех трёх видов аутентификации находятся в проекте auth-demo, в README проекта подробно объясняется, как им пользоваться, можете установить его и посмотреть. Для OAuth 2.0 нужно настроить oauth в google и получить ClientId, для этого можно воспользоваться гайдом https://developers.google.com/identity/protocols/oauth2

В реализации JWT в нашем демо используется сущность Angular **Interceptor** (перехватчик) - подробнее про них вы можете прочитать в [`документации`]([https://portswigger.net/kb/issues/00200901_jwt-none-algorithm-supported](https://angular.dev/guide/http/interceptors)) 
Интерцепторы перехватывают http запрос (по умолчанию - все http запросы) и позволяют изменять его перед отправкой далее. Подключается интерцептор в app.config, который затем используется при бутстраппинге приложения в main.ts, или на уровне роутов, например
