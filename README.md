...
Интерцепторы перехватывают все http запросы и позволяют изменять его перед отправкой далее. Подключается интерцептор в app.config, который затем используется при бутстраппинге приложения в main.ts, или на уровне роутов, например
